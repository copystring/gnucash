 ...
 * adding the ability to show a color indicator at the top right corner
 * to flag the cell.
 */
...
static void gnc_cell_renderer_text_flag_render (GtkCellRenderer* cell,
...
gnc_cell_renderer_text_flag_init (GncCellRendererTextFlag *celltext)
{
    ...
    gdk_rgba_parse (&celltext->color_selected, "red");
...
gnc_cell_renderer_text_flag_class_init (GncCellRendererTextFlagClass *class)
{
    ...
    cell_class->snapshot = gnc_cell_renderer_text_flag_render;

    g_object_class_install_property (object_class, PROP_FLAG_SIZE,
        g_param_spec_int ("flag-size", "Flag size", "Flag size", 0, 50,
                          8, G_PARAM_READWRITE));

    g_object_class_install_property (object_class, PROP_FLAG_COLOR,
        g_param_spec_string ("flag-color", "Flag color name",
                             "Flag color as a string", "red",
                             G_PARAM_WRITABLE));

    g_object_class_install_property (object_class, PROP_FLAG_COLOR_RGBA,
        g_param_spec_boxed ("flag-color-rgba", "Flag color as RGBA",
                            "Flag color as a GdkRGBA", GDK_TYPE_RGBA,
                            G_PARAM_READWRITE));

    g_object_class_install_property (object_class, PROP_FLAG_COLOR_SELECTED,
    ...
    g_object_class_install_property (object_class, PROP_FLAG_COLOR_RGBA_SELECTED,
    ...
    g_object_class_install_property (object_class, PROP_FLAGGED,
 ...
 * coloured indicator as a flag in the top right corner
 *
...
static void
gnc_cell_renderer_text_flag_render (GtkCellRenderer* cell,
                                    GtkSnapshot* snapshot,
                                    GtkWidget* widget,
                                    const GdkRectangle* background_area,
                                    const GdkRectangle* cell_area,
                                    GtkCellRendererState flags)
{
    ...
    GTK_CELL_RENDERER_CLASS(gnc_cell_renderer_text_flag_parent_class)
        ->snapshot (cell, snapshot, widget, background_area, cell_area, flags);

    // add the flag (indicator in the top right corner)
    if (celltext->flagged)
    {
        ...
        // in gtk3 these were triangles, maybe change at a later time
        if (flags & GTK_CELL_RENDERER_SELECTED)
            gtk_snapshot_append_color (snapshot, &celltext->color_selected,
                                       &GRAPHENE_RECT_INIT(x, y, size, size));
        else
            gtk_snapshot_append_color (snapshot, &celltext->color,
                                       &GRAPHENE_RECT_INIT(x, y, size, size));
    }
}
