...
{ const gchar *titles[] = { N_("Type"), N_("Opened"), N_("Closed"), N_("Title"), N_("Balance"), N_("Gains") }; for (guint column = LOT_COLUMN_TYPE; column <= LOT_COLUMN_GAINS; column++) { GtkColumnViewColumn *vc = lot_column_new (_(titles[column]), column); gtk_column_view_append_column (view, vc); } }
static void append_split_columns (GtkColumnView *view)
{ const gchar *titles[] = { N_("Date"), N_("Num"), N_("Description"), N_("Amount"), N_("Value"), N_("Gain/Loss"), N_("Balance") }; for (guint column = SPLIT_COLUMN_DATE; column <= SPLIT_COLUMN_BALANCE; column++) { GtkColumnViewColumn *vc = split_column_new (_(titles[column]), column); gtk_column_view_append_column (view, vc); } }
