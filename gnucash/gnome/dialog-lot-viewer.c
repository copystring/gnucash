...
typedef enum { LOT_COLUMN_TYPE, LOT_COLUMN_OPEN, LOT_COLUMN_CLOSE, LOT_COLUMN_TITLE, LOT_COLUMN_BALANCE, LOT_COLUMN_GAINS } LotColumn;
typedef enum { SPLIT_COLUMN_DATE, SPLIT_COLUMN_NUM, SPLIT_COLUMN_DESCRIPTION, SPLIT_COLUMN_AMOUNT, SPLIT_COLUMN_VALUE, SPLIT_COLUMN_GAIN_LOSS, SPLIT_COLUMN_BALANCE } SplitColumn;
...
{ const LotRow *a = first, *b = second; switch (GPOINTER_TO_UINT (data)) { case LOT_COLUMN_TYPE: return ordering_string (a->type, b->type); case LOT_COLUMN_OPEN: return ordering_int64 (a->open_date, b->open_date); case LOT_COLUMN_CLOSE: return ordering_int64 (a->close_date, b->close_date); case LOT_COLUMN_TITLE: return ordering_string (a->title, b->title); case LOT_COLUMN_BALANCE: return ordering_double (a->balance_sort, b->balance_sort); case LOT_COLUMN_GAINS: return ordering_double (a->gains_sort, b->gains_sort); default: return GTK_ORDERING_EQUAL; } }
static GtkOrdering split_sort_cb (gconstpointer first, gconstpointer second, gpointer data)
{ const SplitRow *a = first, *b = second; switch (GPOINTER_TO_UINT (data)) { case SPLIT_COLUMN_DATE: return ordering_int64 (a->date, b->date); case SPLIT_COLUMN_NUM: return ordering_string (a->number, b->number); case SPLIT_COLUMN_DESCRIPTION: return ordering_string (a->description, b->description); case SPLIT_COLUMN_AMOUNT: return ordering_double (a->amount_sort, b->amount_sort); case SPLIT_COLUMN_VALUE: return ordering_double (a->value_sort, b->value_sort); case SPLIT_COLUMN_GAIN_LOSS: return ordering_double (a->gain_loss_sort, b->gain_loss_sort); case SPLIT_COLUMN_BALANCE: return ordering_double (a->balance_sort, b->balance_sort); default: return GTK_ORDERING_EQUAL; } }

static void lot_setup_cb (G_GNUC_UNUSED GtkListItemFactory *factory, GtkListItem *item, gpointer data)
{ guint column = GPOINTER_TO_UINT (data); GtkWidget *label = gtk_label_new (NULL); gtk_label_set_xalign (GTK_LABEL (label), column == LOT_COLUMN_BALANCE || column == LOT_COLUMN_GAINS ? 1.0f : 0.0f); gtk_label_set_ellipsize (GTK_LABEL (label), PANGO_ELLIPSIZE_END); gtk_list_item_set_child (item, label); }
...
{ LotRow *row = (LotRow *)gtk_list_item_get_item (item); GtkLabel *label = GTK_LABEL (gtk_list_item_get_child (item)); guint column = GPOINTER_TO_UINT (data); gchar *text = NULL; if (!row) { gtk_label_set_text (label, ""); return; } switch (column) { case LOT_COLUMN_TYPE: gtk_label_set_text (label, row->type); break; case LOT_COLUMN_OPEN: text = lot_open_date_text (row->lot); gtk_label_set_text (label, text ? text : ""); g_free (text); break; case LOT_COLUMN_CLOSE: text = lot_close_date_text (row->lot); gtk_label_set_text (label, text ? text : ""); g_free (text); break; case LOT_COLUMN_TITLE: g_object_set_data_full (G_OBJECT (item), "lot-title-binding", g_object_bind_property (row, "title", label, "label", G_BINDING_SYNC_CREATE), lot_title_binding_destroy); break; case LOT_COLUMN_BALANCE: gtk_label_set_text (label, row->balance); break; case LOT_COLUMN_GAINS: gtk_label_set_text (label, row->gains); break; default: break; } }
...
{ guint column = GPOINTER_TO_UINT (data); GtkWidget *label = gtk_label_new (NULL); gtk_label_set_xalign (GTK_LABEL (label), column >= SPLIT_COLUMN_AMOUNT ? 1.0f : 0.0f); gtk_label_set_ellipsize (GTK_LABEL (label), PANGO_ELLIPSIZE_END); if (column == SPLIT_COLUMN_DESCRIPTION) gtk_widget_set_size_request (label, 200, -1); gtk_list_item_set_child (item, label); }
static void split_bind_cb (G_GNUC_UNUSED GtkListItemFactory *factory, GtkListItem *item, gpointer data)
{ SplitRow *row = (SplitRow *)gtk_list_item_get_item (item); GtkLabel *label = GTK_LABEL (gtk_list_item_get_child (item)); guint column = GPOINTER_TO_UINT (data); gchar *date = NULL; if (!row) { gtk_label_set_text (label, ""); return; } switch (column) { case SPLIT_COLUMN_DATE: date = qof_print_date (row->date); gtk_label_set_text (label, date ? date : ""); g_free (date); break; case SPLIT_COLUMN_NUM: gtk_label_set_text (label, row->number); break; case SPLIT_COLUMN_DESCRIPTION: gtk_label_set_text (label, row->description); break; case SPLIT_COLUMN_AMOUNT: gtk_label_set_text (label, row->amount); break; case SPLIT_COLUMN_VALUE: gtk_label_set_text (label, row->value); break; case SPLIT_COLUMN_GAIN_LOSS: gtk_label_set_text (label, row->gain_loss); break; case SPLIT_COLUMN_BALANCE: gtk_label_set_text (label, row->balance); break; default: break; } }
static GtkColumnViewColumn *lot_column_new (const gchar *title, LotColumn column)
{ GtkListItemFactory *factory = GTK_LIST_ITEM_FACTORY (gtk_signal_list_item_factory_new ()); GtkColumnViewColumn *view_column; GtkCustomSorter *sorter; g_signal_connect (factory, "setup", G_CALLBACK (lot_setup_cb), GUINT_TO_POINTER (column)); g_signal_connect (factory, "bind", G_CALLBACK (lot_bind_cb), GUINT_TO_POINTER (column)); if (column == LOT_COLUMN_TITLE) g_signal_connect (factory, "unbind", G_CALLBACK (lot_unbind_cb), NULL); view_column = gtk_column_view_column_new (title, factory); gtk_column_view_column_set_resizable (view_column, TRUE); gtk_column_view_column_set_expand (view_column, column == LOT_COLUMN_TITLE); if (column == LOT_COLUMN_OPEN || column == LOT_COLUMN_CLOSE) gtk_column_view_column_set_fixed_width (view_column, 110); if (column == LOT_COLUMN_TYPE) gtk_column_view_column_set_fixed_width (view_column, 55); if (column == LOT_COLUMN_BALANCE || column == LOT_COLUMN_GAINS) gtk_column_view_column_set_fixed_width (view_column, 115); sorter = gtk_custom_sorter_new (lot_sort_cb, GUINT_TO_POINTER (column), NULL); gtk_column_view_column_set_sorter (view_column, GTK_SORTER (sorter)); g_object_unref (sorter); return view_column; }
static GtkColumnViewColumn *split_column_new (const gchar *title, SplitColumn column)
{ GtkListItemFactory *factory = GTK_LIST_ITEM_FACTORY (gtk_signal_list_item_factory_new ()); GtkColumnViewColumn *view_column; GtkCustomSorter *sorter; g_signal_connect (factory, "setup", G_CALLBACK (split_setup_cb), GUINT_TO_POINTER (column)); g_signal_connect (factory, "bind", G_CALLBACK (split_bind_cb), GUINT_TO_POINTER (column)); view_column = gtk_column_view_column_new (title, factory); gtk_column_view_column_set_resizable (view_column, TRUE); gtk_column_view_column_set_expand (view_column, column == SPLIT_COLUMN_DESCRIPTION); if (column == SPLIT_COLUMN_DATE) gtk_column_view_column_set_fixed_width (view_column, 110); if (column == SPLIT_COLUMN_NUM) gtk_column_view_column_set_fixed_width (view_column, 70); if (column >= SPLIT_COLUMN_AMOUNT) gtk_column_view_column_set_fixed_width (view_column, 100); sorter = gtk_custom_sorter_new (split_sort_cb, GUINT_TO_POINTER (column), NULL); gtk_column_view_column_set_sorter (view_column, GTK_SORTER (sorter)); g_object_unref (sorter); return view_column; }
static void append_lot_columns (GtkColumnView *view)
{ const gchar *titles[] = { N_("Type"), N_("Opened"), N_("Closed"), N_("Title"), N_("Balance"), N_("Gains") }; for (guint column = LOT_COLUMN_TYPE; column <= LOT_COLUMN_GAINS; column++) { GtkColumnViewColumn *vc = lot_column_new (_(titles[column]), column); gtk_column_view_append_column (view, vc); g_object_unref (vc); } }
static void append_split_columns (GtkColumnView *view)
{ const gchar *titles[] = { N_("Date"), N_("Num"), N_("Description"), N_("Amount"), N_("Value"), N_("Gain/Loss"), N_("Balance") }; for (guint column = SPLIT_COLUMN_DATE; column <= SPLIT_COLUMN_BALANCE; column++) { GtkColumnViewColumn *vc = split_column_new (_(titles[column]), column); gtk_column_view_append_column (view, vc); g_object_unref (vc); } }
