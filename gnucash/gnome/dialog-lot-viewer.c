...
static Split *
selected_split (GtkSingleSelection *selection)
{
...
static void
split_selection_changed_cb (G_GNUC_UNUSED GObject *object, G_GNUC_UNUSED GParamSpec *pspec,
                            gpointer user_data)
{
...
{
    GNCLotViewer *lv = user_data;
    Split *split = lv->selected_lot ? selected_split (lv->split_free_selection) : NULL;
    if (!split)
        return;
...
{
    GNCLotViewer *lv = user_data;
    Split *split = lv->selected_lot ? selected_split (lv->split_in_lot_selection) : NULL;
    if (!split || !can_remove_split (split, lv->selected_lot))
        return;
    gnc_suspend_gui_refresh ();
    xaccAccountBeginEdit (lv->account);
...
init_split_view (GNCLotViewer *lv, GtkColumnView *view, GListStore **store,
                 GtkSortListModel **sorted, GtkSingleSelection **selection)
{
    GtkSorter *sorter;
    ...
    *selection = gtk_single_selection_new (G_LIST_MODEL (g_object_ref (*sorted)));
    ...
    g_signal_connect (*selection, "notify::selected", G_CALLBACK (split_selection_changed_cb), lv);
}
static void
